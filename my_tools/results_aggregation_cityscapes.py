...
data_root = 'local_results/ss/cityscapes/pspnet_r50-d8_4xb2-40k_cityscapes-512x1024_suppressor_e5_all_frozen'
...
total_index = ['cityscapes']
total_columns = ['aAcc','mIoU','mAcc']
total_df = pd.DataFrame(data=0, columns=total_columns, index=total_index)
total_df.loc['cityscapes','aAcc'] = round(imc_df_aacc.loc['clean','1'],2)
total_df.loc['cityscapes','mIoU'] = round(imc_df_miou.loc['clean','1'],2)
total_df.loc['cityscapes','mAcc'] = round(imc_df_macc.loc['clean','1'],2)

total_df.loc['cityscapes-C','aAcc'] = round(average_excluding_first_row_aacc,2)
total_df.loc['cityscapes-C','mIoU'] = round(average_excluding_first_row_miou,2)
total_df.loc['cityscapes-C','mAcc'] = round(average_excluding_first_row_macc,2)
